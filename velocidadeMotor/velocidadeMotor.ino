...
void setup() {
    ...
    iotRPM.setup();
    iotRPM.AcaoSetarDadosReceber(DadosReceberVelocidadeMotor);
...
void DadosReceberVelocidadeMotor(long tempoDecorrido)
{
  
  // ms para min (f(x) = x*0,00001667)
  velocidademotor = 1/(tempoDecorrido*0.0000167); // tempoDecorrido = periodo; 1/periodo = frequencia em milisegundos

  Serial.print("tempoDecorrido=");
  Serial.println(tempoDecorrido, 7); 

  Serial.print("velocidademotor=");
  Serial.println(velocidademotor, 7);      
}
